...
const PILLARS: Pillar[] = [
  { name: "Governance", layer: "foundation", definition: "Named owners. Clear decision rights. Accountability that lives in structure, not individuals.", implication: "Without governance, AI makes decisions no one owns. Every automated outcome becomes an unowned risk.", question: "When your AI produces a harmful output tomorrow, who is accountable — and did you know that before it happened?", example: "At Ryan Specialty, governance frameworks were established before AI deployment — not in response to failure. Decision rights preceded automation." },
  { name: "Process", layer: "foundation", definition: "Documented before automated. Governed before scaled. Clear enough to hand to a machine.", implication: "Automating an undocumented process does not fix it. It accelerates it. AI inherits the dysfunction and executes it at speed.", question: "Can you describe, precisely, the process you are about to automate — or does it exist only in the institutional memory of three people?", example: "At Ryan Specialty, process documentation preceded every automation initiative — making the result transferable, auditable, and scalable across 18 acquisitions." },
  { name: "Data", layer: "foundation", definition: "Trusted. Governed. Owned. Every field has an accountable steward.", implication: "AI learns from your data. If your data reflects years of accumulated dysfunction, your AI will too — with confidence.", question: "Would you make a business-critical decision based entirely on your current data? If not, what makes you trust the AI that trains on it?", example: "Data governance established before AI deployment creates a compounding advantage. Organizations that solve this early build an asset that grows more valuable as AI capabilities expand." },
  { name: "Strategy", layer: "direction", definition: "AI investment tied directly to business outcomes the board cares about.", implication: "Technology without strategic alignment accelerates activity in the wrong direction. Speed is not value if the destination is wrong.", question: "Does every AI initiative in your organization trace directly to a business outcome — or to a technology team roadmap?", example: "The organizations extracting the most value from AI made it a business strategy decision before it became a technology one." },
  { name: "Technology", layer: "direction", definition: "Platforms serve the operating model. Not the reverse.", implication: "A technology stack built for vendor defaults rather than the organization operating model creates a ceiling on every transformation initiative.", question: "Is your enterprise platform configured to how your organization actually works — or to how the vendor imagined a generic organization works?", example: "Organizations that treat ServiceNow as a strategic operating layer — not a ticketing system — consistently extract three to five times the value from the same investment." },
  { name: "People", layer: "direction", definition: "Leaders who own outcomes. Not activities.", implication: "Technology transforms at the speed of adoption. Adoption happens at the speed of trust. Without deliberate change leadership, AI becomes the most expensive unused investment in the organization.", question: "Do your leaders know how to lead an organization where AI is a team member — or are they still leading one where it is a project?", example: "The executives who will define technology leadership in the next decade are those who develop AI literacy as a leadership competency, not a technical one." },
];

const VALUE_GROUPS: ValueGroup[] = [
  { label: "Growth", outcomes: [{ title: "Revenue Growth", description: "AI identifies opportunities at scale that human bandwidth cannot sustain. The operating model determines whether those signals reach decisions." }, { title: "Customer Experience", description: "Faster resolution, smarter service, personalized at scale. Only possible when the process layer is clean enough to automate confidently." }] },
  { label: "Efficiency", outcomes: [{ title: "Margin Improvement", description: "Automation removes friction from every process it touches. The organizations with the cleanest processes compound this advantage fastest." }, { title: "Operational Scale", description: "More output from the same cost base. Not by working harder — by building the operating model that lets AI absorb the volume." }] },
  { label: "Resilience", outcomes: [{ title: "Risk Reduction", description: "Governance designed before incidents is the only kind that works. AI governance retrofitted after a failure is crisis management, not leadership." }, { title: "M&A Integration", description: "Every acquisition is an operating model transaction. Organizations with mature frameworks absorb acquisitions. Those without absorb their dysfunction." }] },
];
...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return <p style={{ fontFamily: "system-ui,sans-serif", fontSize: "9px", fontWeight: 600, letterSpacing: "3px", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.3)", margin: "0 0 24px 0" }}>{children}</p>;
}
  ...
  return (
    <section ref={ref} style={{ padding: "140px 0 100px", opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(24px)", transition: "opacity 0.9s ease, transform 0.9s ease" }}>
      ...
      <p style={{ fontFamily: "system-ui,sans-serif", fontSize: "clamp(15px,1.8vw,18px)", color: "rgba(255,255,255,0.72)", margin: "0 0 52px 0", maxWidth: "580px", lineHeight: 1.65 }}>
        Most organizations do not have an AI problem.{" "}<span style={{ color: "#ffffff" }}>They have an operating model problem.</span>
      </p>
      <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" as const }}>
        <a href="#amplifier" style={{ display: "inline-flex", alignItems: "center", padding: "14px 28px", background: "#ffffff", color: "#08080A", fontFamily: "system-ui,sans-serif", fontSize: "13px", fontWeight: 600, letterSpacing: "0.5px", textDecoration: "none", borderRadius: "2px" }}>Explore the Framework</a>
        <a href="mailto:imanzamanii@yahoo.com" style={{ display: "inline-flex", alignItems: "center", padding: "14px 28px", background: "transparent", color: "rgba(255,255,255,0.6)", fontFamily: "system-ui,sans-serif", fontSize: "13px", fontWeight: 500, letterSpacing: "0.5px", textDecoration: "none", border: "1px solid rgba(255,255,255,0.15)", borderRadius: "2px" }}>Get in Touch</a>
      </div>
    </section>
  );
}
    ...
    <section id="amplifier" style={{ padding: "80px 0 100px" }}>
      ...
      <div ref={ref} style={{ display: "flex", gap: "2px", flexWrap: "wrap" as const }}>
        <div style={{ flex: 1, minWidth: "260px", padding: "40px 36px", borderRadius: "2px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(-32px)", transition: "opacity 0.8s ease, transform 0.8s ease" }}>
          ...
          {["Unclear governance.", "Undocumented process.", "Data no one trusts."].map(l => <p key={l} style={{ fontFamily: "Georgia,serif", fontSize: "15px", fontStyle: "italic", color: "rgba(255,255,255,0.32)", margin: "0 0 6px 0", lineHeight: 1.5 }}>{l}</p>)}
        ...
        <div style={{ flex: 1, minWidth: "260px", padding: "40px 36px", borderRadius: "2px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.18)", borderTop: "2px solid rgba(255,255,255,0.7)", opacity: inView ? 1 : 0, transform: inView ? "translateX(0)" : "translateX(32px)", transition: "opacity 0.8s ease 0.15s, transform 0.8s ease 0.15s" }}>
          ...
          {["Clear governance.", "Defined process.", "Data with owners."].map(l => <p key={l} style={{ fontFamily: "Georgia,serif", fontSize: "15px", fontStyle: "italic", color: "rgba(255,255,255,0.75)", margin: "0 0 6px 0", lineHeight: 1.5 }}>{l}</p>)}
...
function PillarCard({ pillar }: { pillar: Pillar }) {
  ...
  return (
    <div role="button" tabIndex={0} aria-expanded={open} onClick={() => setOpen(!open)} onKeyDown={e => e.key === "Enter" && setOpen(!open)}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <p style={{ fontFamily: "system-ui,sans-serif", fontSize: "9px", fontWeight: 600, letterSpacing: "2px", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.3)", margin: "0 0 8px 0" }}>{pillar.layer === "foundation" ? "Foundation" : "Direction"}</p>
          <h3 style={{ fontFamily: "Georgia,serif", fontSize: "20px", fontWeight: 400, color: "#ffffff", margin: 0, letterSpacing: "-0.01em" }}>{pillar.name}</h3>
        </div>
        <span style={{ color: "rgba(255,255,255,0.4)", fontSize: "20px", transform: open ? "rotate(45deg)" : "rotate(0)", transition: "transform 0.25s ease", marginTop: "2px", lineHeight: 1, display: "inline-block" }}>+</span>
      </div>
      ...
      {open && (
        <div style={{ marginTop: "24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "28px" }}>
            {[{ label: "Definition", content: pillar.definition, quote: false }, { label: "Business Implication", content: pillar.implication, quote: false }, { label: "Executive Question", content: pillar.question, quote: true }, { label: "In Practice", content: pillar.example, quote: false }].map(({ label, content, quote }) => (
              <div key={label}>
                ...
                <p style={{ fontFamily: quote ? "Georgia,serif" : "system-ui,sans-serif", fontSize: "14px", fontStyle: quote ? "italic" : "normal", color: quote ? "rgba(255,255,255,0.72)" : "rgba(255,255,255,0.55)", margin: 0, lineHeight: 1.65 }}>{quote ? `"${content}"` : content}</p>
              </div>
            ))}
          </div>
        </div>
...
function Pillars() {
  ...
  const LayerLabel = ({ text, sub }: { text: string; sub: string }) => (
    <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "16px" }}>
      <p style={{ fontFamily: "system-ui,sans-serif", fontSize: "9px", fontWeight: 600, letterSpacing: "2.5px", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.3)", margin: 0, whiteSpace: "nowrap" as const }}>{text} — {sub}</p>
  ...
  return (
    <section style={{ padding: "80px 0 100px" }}>
      ...
      <h2 style={{ fontFamily: "Georgia,serif", fontSize: "clamp(24px,3.5vw,38px)", fontWeight: 400, color: "#ffffff", margin: "0 0 16px 0", letterSpacing: "-0.015em", lineHeight: 1.2, maxWidth: "600px" }}>The foundation determines the ceiling.</h2>
      <p style={{ fontFamily: "system-ui,sans-serif", fontSize: "15px", color: "rgba(255,255,255,0.45)", margin: "0 0 64px 0", maxWidth: "520px", lineHeight: 1.65 }}>Six pillars. Two layers. Click any pillar to expand.</p>
      <div ref={ref} style={{ opacity: inView ? 1 : 0, transition: "opacity 0.7s ease" }}>
        <div style={{ marginBottom: "40px" }}>
          <LayerLabel text="Foundation" sub="Can the organization execute?" />
          <div style={{ display: "flex", flexDirection: "column" as const, gap: "2px" }}>{foundation.map(p => <PillarCard key={p.name} pillar={p} />)}</div>
        </div>
        <div>
          <LayerLabel text="Direction" sub="Is it aimed at the right destination?" />
          <div style={{ display: "flex", flexDirection: "column" as const, gap: "2px" }}>{direction.map(p => <PillarCard key={p.name} pillar={p} />)}</div>
        </div>
      </div>
    </section>
  );
...
function ReadinessFlow() {
  const { ref, inView } = useInView(0.2);
  const steps = [
    { label: "Operational Readiness", desc: "The organization executes consistently. Accountability is structural. AI has a foundation it can run on.", strong: false },
  ...
  return (
    <section style={{ padding: "80px 0 100px" }}>
      ...
      <h2 style={{ fontFamily: "Georgia,serif", fontSize: "clamp(24px,3.5vw,38px)", fontWeight: 400, color: "#ffffff", margin: "0 0 60px 0", letterSpacing: "-0.015em", lineHeight: 1.2, maxWidth: "560px" }}>Readiness is sequential.<br />You cannot skip the foundation.</h2>
        ...
        {steps.map((step, i) => (
          <div key={step.label}>
            <div style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(20px)", transition: `opacity 0.7s ease ${i * 0.2}s, transform 0.7s ease ${i * 0.2}s` }}>
              <div style={{ padding: "32px 36px", border: `1px solid rgba(255,255,255,${step.strong ? "0.18" : "0.12"})`, borderTop: `${step.strong ? 2 : 1}px solid rgba(255,255,255,${step.strong ? "0.5" : "0.3"})`, borderRadius: "2px", background: step.strong ? "rgba(255,255,255,0.04)" : "transparent" }}>
